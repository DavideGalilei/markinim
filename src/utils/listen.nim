...
var
  pendingMessages: Table[string, Record[Update]] # (uuid): ((userId, chatId), Update Future)


proc listenUpdater*(bot: Telebot, update: Update): Future[bool] {.async.} =
...
proc getMessage*(userId, chatId: int64, timeout: int = DEFAULT_TIMEOUT): Future[Update] {.async.} =
